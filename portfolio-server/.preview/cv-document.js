...
exports.prepareCvPhoto = prepareCvPhoto;
exports.buildCvDocument = buildCvDocument;
/**
...
const { Document, Page, View, Text, Link, Image, Font, StyleSheet, } = require('@react-pdf/renderer');
...
try {
    ...
    console.warn('CV: police Roboto introuvable, fallback Helvetica —', e.message);
}
try {
...
function bucketSkills(skills) {
    ...
    skills.forEach((s) => {
        var _a, _b;
        const cat = ((_a = s.coding) === null || _a === void 0 ? void 0 : _a.category) || 'tools';
        const k = map[cat] || 'tools';
        const n = (_b = s.coding) === null || _b === void 0 ? void 0 : _b.name;
        if (n)
            (out[k] = out[k] || []).push(n);
    });
    return order.filter((k) => out[k]).map((k) => ({ key: k, vals: out[k] }));
}
const httpify = (u) => (!u ? u : (u.startsWith('http') ? u : `https://${u}`));
const strip = (u) => (u || '').replace(/^https?:\/\//, '').replace(/^www\./, '').replace(/\/$/, '');
...
function joinDesc(e) {
    ...
    return arr.filter(Boolean).map((d) => String(d).replace(/\.$/, '')).join(' · ');
}
function buildLines(me) {
    const lines = [];
    const sp = (t, c = 'punc') => ({ t, c });
    const str = (t) => ({ t: `"${t}"`, c: 'str' });
    const lnk = (t, href) => ({ t: `"${t}"`, c: 'str', href });
    const push = (indent, ...spans) => lines.push({ indent, spans });
    const blank = () => lines.push({ indent: 0, spans: [] });
    const kv = (indent, key, val) => push(indent, sp(key, 'key'), sp(': '), val, sp(','));
    ...
    if (sub)
        push(0, sp(` * ${sub}`, 'com'));
    if (me.website)
        push(0, sp(` * @see ${strip(me.website)}`, 'com'));
    push(0, sp(' */', 'com'));
    blank();
    ...
    kv(1, 'role', str(role));
    if (me.city)
        kv(1, 'location', str(me.city));
    if (age)
        push(1, sp('age', 'key'), sp(': '), sp(String(age), 'num'), sp(','));
    blank();
    /* contact */
    push(1, sp('contact', 'key'), sp(': '), sp('{'));
    if (me.email)
        kv(2, 'email', lnk(me.email, `mailto:${me.email}`));
    if (me.phoneNumber)
        kv(2, 'phone', str(me.phoneNumber));
    if (me.github)
        kv(2, 'github', lnk(strip(me.github), httpify(me.github)));
    if (me.linkedin)
        kv(2, 'linkedin', lnk(strip(me.linkedin), httpify(me.linkedin)));
    if (me.website)
        kv(2, 'website', lnk(strip(me.website), httpify(me.website)));
    push(1, sp('},'));
    blank();
    ...
    if (buckets.length) {
        ...
        buckets.forEach(({ key, vals }) => {
            const spans = [sp(key, 'key'), sp(': '), sp('[')];
            vals.forEach((n, idx) => { spans.push(str(n)); if (idx < vals.length - 1)
                spans.push(sp(', ')); });
            spans.push(sp('],'));
            push(2, ...spans);
        });
    ...
    if (langs.length) {
        const spans = [sp('languages', 'key'), sp(': '), sp('[')];
        langs.forEach((l, idx) => {
    ...
    push(0, sp('};'));
    blank();
    push(0, sp('export default', 'kw'), sp(' paul'), sp(';'), sp('   // ', 'com'), sp('disponible · open to work', 'com'));
    return lines;
}
/* ---- Rendu ---- */
function renderLine(ln, i) {
    const codeChildren = ln.spans.length
        ...
        : ' ';
    return h(View, { key: i, style: S.row }, h(Text, { style: S.gutter }, String(i + 1)), h(Text, { style: [S.code, { marginLeft: ln.indent * 13 }] }, codeChildren));
}
function Avatar(me) {
 ...
 */
async function prepareCvPhoto(srcPath) {
    if (!srcPath)
        return null;
    try {
        ...
        const c = size / 2;
        const ring = 10;
        const pad = 6; // marge transparente : l'anneau ne touche pas le bord du canvas
        const R = c - pad;
        const mask = Buffer.from(`<svg width="${size}" height="${size}"><circle cx="${c}" cy="${c}" r="${R}" fill="#fff"/></svg>`);
        const ringSvg = Buffer.from(`<svg width="${size}" height="${size}"><circle cx="${c}" cy="${c}" r="${R - ring / 2}" fill="none" stroke="${UI.accent}" stroke-width="${ring}"/></svg>`);
        ...
        console.warn('CV: détourage photo impossible —', e.message);
        return null;
    }
}
function TitleBar() {
    return h(View, { style: S.titlebar }, h(View, { style: [S.dot, { backgroundColor: '#ff5f56' }] }), h(View, { style: [S.dot, { backgroundColor: '#ffbd2e' }] }), h(View, { style: [S.dot, { backgroundColor: '#27c93f', marginRight: 0 }] }), h(Text, { style: S.winTitle }, 'paul-richez.ts — Visual Studio Code'), h(View, { style: S.winTitleSpacer }));
}
function TabBar() {
    return h(View, { style: S.tabbar }, h(View, { style: S.tab }, h(View, { style: S.tsIcon }, h(Text, { style: S.tsIconTxt }, 'TS')), h(Text, { style: S.tabTxt }, 'paul-richez.ts'), h(Text, { style: S.tabClose }, '×')), h(View, { style: S.tabInactive }, h(Text, { style: S.tabTxtInactive }, 'README.md'), h(Text, { style: S.tabClose }, '×')));
}
function StatusBar(lineCount) {
    return h(View, { style: S.statusbar }, h(Text, { style: S.stTxt }, 'main • disponible'), h(Text, { style: S.stTxt }, `Ln ${lineCount}, Col 1   Spaces: 2   UTF-8   LF   TypeScript`));
}
/* ---- Document ---- */
function buildCvDocument(me) {
    if (!me)
        throw new Error('Données CV manquantes');
    const fullName = `${me.firstName || ''} ${me.lastName || ''}`.trim();
    const lines = buildLines(me);
    return h(Document, { title: `${fullName} — CV`, author: fullName, creator: fullName }, h(Page, { size: 'A4', style: S.page }, h(View, { style: S.win }, TitleBar(), TabBar(), h(View, { style: S.editor }, lines.map(renderLine)), StatusBar(lines.length), Avatar(me))));
}
